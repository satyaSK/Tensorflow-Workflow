...
def create_directory(path_to_checkpoints):
    try:
...
with tf.Session() as sess:
	...
	print("\nGood To Go - training has started from batch number {0}\n".format(initial_step))
	for i in tqdm(range(epochs)):
		epoch_loss = 0

		for _ in range(initial_step, initial_step + num_batches):
...
##Added checkpoints to latest version!
